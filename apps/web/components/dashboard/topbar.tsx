...
export function Topbar({ title }: { title: string }) {
  ...
  return (
    <header className="flex h-12 shrink-0 items-center gap-3 border-b border-huma-bdr bg-huma-bg2 px-4 transition-all duration-300">
      <div>
      ...
      <div className="ml-auto flex items-center gap-2">
                ...
                <div key={n.title} className="flex gap-2.5 border-b border-huma-bdr2 px-3.5 py-2.5 last:border-0">
                  <div
                    className={`mt-1 h-1.5 w-1.5 shrink-0 rounded-full ${
                      n.type === 'err' ? 'bg-huma-err' : 'bg-huma-warn'
                    }`}
                  />
                  <div>
        ...
        <button type="button" className="btn-primary" onClick={() => api.resumeAll()}>▶ 발행 실행</button>
      </div>
    </header>
  );
}
