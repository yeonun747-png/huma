...
export function AccountList() {
  ...
  const [form, setForm] = useState({ name: '', naver_id: '', naver_pw: '', account_type: 'posting' as AccountType, blog_url: '' });
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.createAccount({
        ...
        is_active: true,
        health_score: 100,
        blog_index: form.account_type === 'posting' ? 5 : 0,
        wpm: 45,
      });
      setForm({ name: '', naver_id: '', naver_pw: '', account_type: 'posting', blog_url: '' });
      load();
    } catch (e) {
        ...
        accounts.map((ac) => (
          <div key={ac.id} className="rounded-lg border border-huma-bdr bg-huma-bg3 p-3 hover:border-huma-acc">
            <div className="mb-2 flex items-center gap-2">
              ...
              <Button size="sm" variant="ghost" className="text-huma-err" onClick={() => handleDelete(ac)}>
                삭제
              </Button>
            </div>
