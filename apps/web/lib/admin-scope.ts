/** 사이드바·계정 UI 사업 단위 — 연운 / 퀴즈오아시스 / 파나나 분리 */
export type BusinessUnit = 'yeonun' | 'quizoasis' | 'panana';
...
export function getAccessibleBusinessUnits(admin: AdminScope | null): BusinessUnit[] {
  ...
  if (admin.isSuper === true || id === 'superadmin') {
    return ['yeonun', 'quizoasis', 'panana'];
  }
...
export function workspaceToBusinessUnit(workspace: string): BusinessUnit {
  if (workspace === 'quizoasis' || workspace === 'panana' || workspace === 'yeonun') {
    return workspace;
  }
  return 'yeonun';
}
