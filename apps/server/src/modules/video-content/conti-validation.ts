...
export function buildGenericActionBatchFeedback(shotNumbers: number[], narrativeProse?: string): string {
  const nums = shotNumbers.join(', ');
  const forbidden = GENERIC_DEFAULT_ACTION_PHRASES.map((p) => `"${p}"`).join(', ');
  let msg =
    `샷 ${nums} action이 ${forbidden} 등 **빈 filler**이다. ` +
    '각 샷마다 3a narrativeProse 해당 구간의 **구체적** 사건·표정·동작·발견(폰 확인·알림·반응·표정 등)을 action에 쓸 것. ' +
    'filler action으로 대체하거나 유지하지 말 것.';
  const prose = narrativeProse?.trim();
  if (prose) {
    msg += `\n\n3a narrativeProse (샷 action은 아래 서술을 구간별로 구체화):\n${prose}`;
  }
  return msg;
}
