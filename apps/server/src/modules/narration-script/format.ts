...
export function validateNarrationTitle(
  ...
  if (t.length > 42) {
    return { ok: false, message: '제목이 너무 깁니다 — 42자 이내 숏폼 후킹' };
  }
...
export function buildTitlePromptBlock(axisType: NarrationAxisType, topicLabel: string): string {
  const prefix = buildAxisTitlePrefix(axisType);
  const axisWord = axisTitleKeyword(axisType);
  const examples =
    axisType === 'zodiac'
      ? [
          `띠별로 알아보는 ${topicLabel}, 12띠 중 당신은?`,
          `재회운 강한 띠 1위는? 띠별 ${topicLabel}`,
        ]
      : axisType === 'constellation'
        ? [
            `별자리로 알아보는 ${topicLabel}`,
            `재회 가능성 높은 별자리 1위는? ${topicLabel}`,
          ]
        : [
            `연령대별로 알아보는 ${topicLabel}, 내 나이대는?`,
            `40대만 해당? 연령대별 ${topicLabel}`,
          ];

  return `제목 (가장 중요 — 클릭·시청을 결정):
- 반드시 "${prefix}" 또는 "${axisWord}" 축이 제목에 보이게
- 상품 주제「${topicLabel}」를 자연스럽게 포함
- 설명형 부제만 쓰지 말 것 (예: "한눈에", "살펴보세요" 단독 금지)
- 질문·숫자·1위·당신은·반전·궁금증 중 1개 이상
- 12~38자, 숏폼 썸네일·첫 화면용 한 줄
