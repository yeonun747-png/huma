...
describe('narration format', () => {
  ...
  it('requires axis and period in title', () => {
    const bad = validateNarrationTitle('초년부터 말년까지, 내 평생의 흐름을 한눈에', 'zodiac', 'daily');
    expect(bad.ok).toBe(false);
  });

  it('accepts constellation axis title pattern with period', () => {
    const ok = validateNarrationTitle(
      '오늘 별자리로 알아보는 그 사람과 다시 만날 수 있을까',
      'constellation',
      'daily',
    );
  ...
  it('builds fallback title with axis prefix and period', () => {
    expect(
      buildFallbackNarrationTitle('그 사람과 다시 만날 수 있을까', 'constellation', 'full_cover', 'daily'),
    ).toBe('오늘 별자리로 알아보는 그 사람과 다시 만날 수 있을까');
  });
});
