...
export async function uniquifyImageFromUrl(url: string): Promise<string> {
  ...
  const outPath = join(tmpdir(), `huma_${Date.now()}_${randomBetween(1000, 9999)}.jpg`);
...
async function injectRandomExif(filePath: string) {
  try {
    ...
    exifObj['0th'][piexif.ImageIFD.Make] = 'HUMA';
    exifObj['0th'][piexif.ImageIFD.Model] = `Device-${randomBetween(100, 999)}`;
