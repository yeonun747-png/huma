-- reserve_posting_quota_slot: completed_at 대신 KST 실제 발행일 집계 (post-blog-publish-day.ts 와 동일)
...
BEGIN
  ...
  IF COALESCE(v_ps->>'_reconciled_from_failed', 'false') = 'true' THEN
    RETURN NULL;
  END IF;

  IF p_job.completed_at IS NOT NULL THEN
    RETURN (p_job.completed_at AT TIME ZONE 'Asia/Seoul')::date;
  END IF;
