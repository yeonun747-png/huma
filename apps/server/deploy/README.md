...
| 파일 | 용도 |
|------|------|
| `ecosystem.config.cjs` | PM2 (Xvfb + server) — **x11vnc는 systemd** |
| `huma-x11vnc.service` | systemd — VNC :5900 (재부팅 자동) |
| `setup-x11vnc-systemd.sh` | x11vnc systemd 원클릭 등록 |
| `scripts/fix-vnc.sh` | VNC 복구 + RFB 확인 |
...
| `setup-tailscale.sh` | Tailscale — 집 밖 VNC (Personal 무료) |
| `scripts/print-tailscale-vnc.sh` | Tailscale IP → HUMA_VNC_URL 출력 |
...
# apps/server/.env 작성
# Supabase: schema.sql → v3_5 → … → v3_21 → v3_22
```
...
# VNC (RealVNC 5900) — systemd (재부팅 후에도 자동)
sudo HUMA_USER=$USER bash setup-x11vnc-systemd.sh
bash scripts/fix-vnc.sh   # RFB 003.008 확인
```
...
## D. Tailscale (집 밖 RealVNC)

LAN(`172.30.x.x`)은 같은 Wi‑Fi에서만 VNC 가능. **Tailscale Personal(무료)** 로 i7·PC를 같은 tailnet에 두면 집 밖에서도 Direct 접속.

```bash
# i7 Ubuntu
cd ~/huma/apps/server/deploy
bash setup-tailscale.sh
# → .env HUMA_VNC_URL_YEONUN=vnc://100.x.x.x:5900 자동 갱신

pm2 restart huma-server --update-env
bash scripts/print-tailscale-vnc.sh   # IP 재확인
```

**Windows PC**

1. [Tailscale for Windows](https://tailscale.com/download/windows) 설치
2. i7과 **같은 계정** 로그인
3. RealVNC Viewer → **Direct** → `100.x.x.x:5900` (setup-tailscale.sh 출력값)

> x11vnc는 `-listen 0.0.0.0:5900` — Tailscale IP로 접속 가능. **5900 포트포워딩·공인 IP 노출 불필요.**  
> huma API는 Cloudflare Tunnel, VNC 화면만 Tailscale.

## E. LTE 모뎀 10슬롯 (v3.10 확정)
